...
function TransactionItem({transaction}) {
  ...
  return (
    <tr className='group border-b-2 border-slate-100 transition-all hover:bg-slate-50'>
      ...
      <td className='text-nowrap p-4 text-sm text-slate-600'><span className={`px-2 py-1 rounded bg-slate-100 group-hover:bg-slate-200 transition-all`}>{`${category[0].toUpperCase()}${category.slice(1)}`}</span></td>
